...
class NLTKUse(object):
    def __init__(self, file_path: str):
        ...
        self.data = DATA
        self.all_words = set(word.lower()
                             for passage in self.data for word in word_tokenize(passage[0]) if word.lower() not in self.stop_words)
    ...
    def train(self):
        ...
        training_data = [({word: (word in word_tokenize(x[0]))
                           for word in self.all_words}, x[1]) for x in self.data]

        test_data = TEST_DATA

        test_words = set(word.lower()
                         for passage in test_data for word in word_tokenize(passage[0]) if word.lower() not in self.stop_words)

        testing_data = [({word: (word in word_tokenize(x[0]))
                          for word in test_words}, x[1]) for x in test_data]
    ...
    def text(self, text: str):
        ...
        test_sent_features = {word: (word in word_tokenize(
            test_sentence.lower())) for word in self.all_words}
